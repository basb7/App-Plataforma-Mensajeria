...
@app.route('/restablecerclave', methods=['GET','POST'])
...
def cambio_clave():
    ...
    p2=datos['passwd2']
    p1enc=generate_password_hash(p1)
    if controlador.validar_email(usu):
        if p1!=p2:
            flash('Contraseñas no Coinciden')
        elif len(p1)<8:
            flash('las contraseñas no cumplen los niveles de seguridad')
            ...
            resultado=controlador.restablecer_clave(p1enc,usu)
            if resultado:
                flash('La contraseña ha sido actualizada con exito')
                return render_template('login.html')
            else:
                flash("No ha sido posibles cambiar la contraseña")   
        
        return render_template('new-password.html', datauser=usu)
    return render_template('new-password.html', datauser=usu)  
...
@app.route('/new-password/<usuario>')
@app.route('/new-password')
def menu_cambio_password(usuario=None):
    if usuario!=None:
        return render_template('new-password.html', datauser=usuario)
    else:
        return render_template('new-password.html', datauser=usuario)
